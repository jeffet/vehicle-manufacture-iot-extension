  ...
  .controller('DashboardCtrl', [
    ...
    function ($scope, $http, $interval) {
      ...
      $http
        .get(baseOrderUrl, {
          headers: {
            Authorization: `Basic ${btoa('production:productionpw')}`,
          },
        })
            ...
            if (Array.isArray(response.data)) {
              ...
              if ($scope.orders.length > 0) {
                ...
                history.forEach((historicState) => {
                  handleUpdateOrderEvent(
                    historicState.timestamp,
                    historicState.value.orderStatus,
                    historicState.value.id
                  );
                });
              }
            }
      ...
      async function getOrderHistory(orderId) {
        const resp = await new Promise((resolve, reject) => {
          return $http
            .get(`${baseOrderUrl}/${orderId}/history`, {
              headers: {
                Authorization: `Basic ${btoa('production:productionpw')}`,
              },
            })
      ...
      var generateVIN = function () {
        ...
        const yearChar =
          yearChars[(new Date().getFullYear() - 1980) % yearChars.length];
      ...
      var updateOrderStatus = async function (orderId, count) {
        ...
        $http
          .put(`${baseOrderUrl}/${orderId}/status`, status, {
            headers: {
              Authorization: 'Basic ' + btoa('production:productionpw'),
            },
          })
