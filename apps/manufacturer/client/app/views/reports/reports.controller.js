...
angular
  ...
  .controller('ReportsCtrl', [
    ...
    function ($scope, $http, $interval) {
      ...
      const headers = {
        headers: { Authorization: `Basic ${btoa('reports:reportspw')}` },
      };
        ...
        if (event) {
          ...
          if (targetVehicle) {
            $scope.usageEvents.forEach((usageEvent) => {
              ...
              if (
                vehicle.vehicleDetails.makeId ===
                  targetVehicle.vehicleDetails.makeId &&
                vehicle.vehicleDetails.modelType ===
                  targetVehicle.vehicleDetails.modelType &&
      ...
      async function getData() {
        try {
          ...
          const usageEvents = await $http.get(
            `${baseUrl}/vehicles/usage`,
            headers
          );
          ...
          $scope.usageEvents = usageEvents.data
            .filter((event) => {
      ...
      function setupPlaceOrderListener() {
        const orderUpdates = new EventSource(
          baseUrl + '/vehicles/usage/events/added'
        );
        ...
        orderUpdates.onmessage = async (evt) => {
          ...
          if (data.eventType === 2) {
            if (!$scope.vehicles.hasOwnProperty(data.vin)) {
              const vehicle = (
                await $http.get(`${baseUrl}/vehicles/${data.vin}`, headers)
              ).data;
              vehicle.serialNumber = generateSN();
