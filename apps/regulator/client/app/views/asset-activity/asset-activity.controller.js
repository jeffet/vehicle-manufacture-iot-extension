...
angular
  ...
  .controller('AssetActivityCtrl', [
    ...
    function ($scope, $http) {
      ...
      $http
        .get(txUrl, {
          headers: { Authorization: `Basic ${btoa('audit:auditpw')}` },
        })
        ...
        block.transactions.forEach((transaction) => {
          if (transaction.contract === 'org.acme.vehicle_network.vehicles') {
            const {
              ...
              validator_2,
              sign,
            } = get_activity_from_type(transaction.name);
            ...
            $scope.transactions.push({
              ...
              transaction_submitter:
                transaction.caller.identity + '@' + transaction.caller.org,
              transaction_class: isNew ? 'new-row' : 'existing-row',
              block_number: block.number,
              additional_info:
                transaction.name === 'addUsageEvent'
                  ? EventTypes[transaction.parameters.eventType]
                  : null,
            });
          }
        });
      ...
      function get_activity_from_type(type) {
        ...
        switch (type) {
          case 'createPolicy':
            activity = 'New Insurance Issued';
            validator_1 = 'Insurer';
            validator_2 = 'Vehicle Owner';
            break;
          case 'placeOrder':
            activity = 'New Vehicle Request';
            validator_1 = 'Vehicle Owner';
            validator_2 = 'Manufacturer';
            break;
          case 'scheduleOrderForManufacture':
            activity = 'Vehicle Manufacture (Manufacture Scheduled)';
            validator_1 = 'Manufacturer';
            validator_2 = '';
            sign = '';
            break;
          case 'registerVehicleForOrder':
            activity = 'Vehicle Manufacture (Vin Assigned)';
            validator_1 = 'Manufacturer';
            validator_2 = 'Regulator';
            break;
          case 'assignOwnershipForOrder':
            activity = 'Vehicle Manufacture (Owner Assigned)';
            validator_1 = 'Manufacturer';
            validator_2 = '';
            sign = '';
            break;
          case 'deliverOrder':
            activity = 'Vehicle Manufacture (Delivered)';
            validator_1 = 'Vehicle Owner';
            validator_2 = 'Vehicle';
            break;
          case 'addUsageEvent':
